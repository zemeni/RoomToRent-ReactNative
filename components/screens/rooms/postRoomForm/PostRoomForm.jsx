...
const PostRoomForm = ({onSubmit, onCancel}) => {
  ...
  const handleNumericChange = (value, roomId, field) => {
    ...
    const updatedRooms = rooms.map((room) =>
      room.id === roomId ? { ...room, [field]: numericValue } : room
    );
  ...
  const pickImage = async (roomId) => {
    ...
    if (!result.canceled) {
      ...
      const updatedRooms = rooms.map((room) => {
        if (room.id === roomId) {
          const limitedImages = [...room.images, ...newImages].slice(
            0,
            MAX_IMAGES
          );
  ...
  const removeImage = (roomId, uri) => {
    const updatedRooms = rooms.map((room) => {
      if (room.id === roomId) {
        return {
          ...room,
          images: room.images.filter((image) => image !== uri),
        };
      }
      return room;
    });
    ...
    <View key={room.id} style={styles.roomContainer}>
      ...
      <View style={styles.rowContainer}>
        <View style={styles.inputContainer}>
          ...
          <TextInput
            style={[
              styles.input,
              validationErrors[room.id]?.price && styles.errorInput,
            ]}
            placeholder="Enter Price"
            value={room.price.toString()}
            onChangeText={(price) =>
              handleNumericChange(price, room.id, "price")
            }
        ...
        <View style={styles.inputContainer1}>
          ...
          <View style={styles.pickerContainer}>
            <Picker
              ...
              style={styles.picker}
              onValueChange={(value) =>
                handleTextChange(value, room.id, "including")
              }
            >
        ...
        <View style={styles.inputContainer}>
          ...
          <View style={styles.pickerContainer}>
            <Picker
              ...
              style={styles.picker}
              onValueChange={(value) =>
                handleTextChange(value, room.id, "roomType")
              }
            >
        ...
        <View style={styles.inputContainer}>
          ...
          <View style={styles.pickerContainer}>
            <Picker
              ...
              style={styles.picker}
              onValueChange={(value) =>
                handleTextChange(value, room.id, "furnished")
              }
            >
              <Picker.Item label="Yes" value="1" />
              <Picker.Item label="No" value="0" />
      ...
      <TextInput
        ...
        onChangeText={(description) =>
          handleTextChange(description, room.id, "description")
        }
        multiline
      />
      {validationErrors[room.id]?.description && (
        <Text style={styles.errorText}>
          Description must be greater than 50 letters
        </Text>
      )}
      ...
      <TextInput
        style={[
          styles.input,
          validationErrors[room.id]?.bathrooms && styles.errorInput,
        ]}
        placeholder="Enter Number of Bathrooms"
        value={room.bathrooms.toString()}
        onChangeText={(bathrooms) =>
          handleNumericChange(bathrooms, room.id, "bathrooms")
        }
        keyboardType="numeric"
      />
      ...
      <TextInput
        style={[
          styles.input,
          validationErrors[room.id]?.parkings && styles.errorInput,
        ]}
        placeholder="Enter Number of Parkings"
        value={room.parkings.toString()}
        onChangeText={(parkings) =>
          handleNumericChange(parkings, room.id, "parkings")
        }
        keyboardType="numeric"
      />
      {validationErrors[room.id]?.parkings && (
        <Text style={styles.errorText}>
          Number of parking must be 0 or more.
        </Text>
      )}
      ...
      >
        <Text>
          {room.startDate ? room.startDate.toLocaleDateString() : "Select Date"}
        </Text>
      </TouchableOpacity>
      <DateTimePickerModal
        ...
        mode="date"
        onConfirm={(startDate) =>
          handleTextChange(startDate, room.id, "startDate")
        }
        ...
        {room.images.map((uri) => (
          <View key={uri} style={styles.imagePreview}>
            ...
            <TouchableOpacity
              ...
              onPress={() => removeImage(room.id, uri)}
            >
        ...
        ListHeaderComponent={
          <>
            ...
            <GooglePlacesAutocomplete
              ...
              query={{
                key: "AIzaSyCIajIObfer1l1-Jak0Fqk3r0WC7V8Kz_Q",
                language: "en",
          ...
          rooms.length > 0 && (
            <View style={styles.buttonContainer}>
              <TouchableOpacity style={styles.button} onPress={addRoom}>
                <Text style={styles.buttonText}>{`Add Room ${
                  rooms.length + 1
                }`}</Text>
              </TouchableOpacity>
              <TouchableOpacity
